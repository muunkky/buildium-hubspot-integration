    ...
    try {
        ...
        try {
            ...
            if (contactAssocResponse.data.results && contactAssocResponse.data.results.length > 0) {
                console.log(`✅ Contact has ${contactAssocResponse.data.results.length} listing associations:`);
                ...
                console.log('❌ Contact has no listing associations');
            }
        } catch (error) {
            console.log('❌ Error checking contact associations:', error.response?.data?.message || error.message);
        }
        ...
        try {
            ...
            if (listingAssocResponse.data.results && listingAssocResponse.data.results.length > 0) {
                console.log(`✅ Listing has ${listingAssocResponse.data.results.length} contact associations:`);
                ...
                console.log('❌ Listing has no contact associations');
            }
        } catch (error) {
            console.log('❌ Error checking listing associations:', error.response?.data?.message || error.message);
        }
        ...
        try {
            ...
            console.log('✅ Association recreated successfully!');
            console.log('Status:', createResponse.data.status);
            if (createResponse.data.results) {
                console.log('Results:', createResponse.data.results.length, 'associations processed');
            }
        } catch (error) {
            console.log('❌ Error recreating association:', error.response?.data?.message || error.message);
