...
async function testPropertyFiltering() {
    try {
        ...
        console.log('[SEARCH] Testing property filtering with known property IDs...');
        
        // Test with property 57543 (we know it has units from previous output)
        console.log('\n[ITEM] Testing with property 57543 (known to have units):');
        const units57543 = await integration.buildiumClient.getAllUnits(10, 0, [57543]);
        console.log(`[STATS] Found ${units57543.length} units for property 57543`);
        ...
        if (units57543.length > 0) {
            ...
            console.log(`[OK] Units correctly filtered: ${correctProperty.length}/${units57543.length}`);
        }
        
        // Now test with property 140054
        console.log('\n[ITEM] Testing with property 140054:');
        const units140054 = await integration.buildiumClient.getAllUnits(10, 0, [140054]);
        console.log(`[STATS] Found ${units140054.length} units for property 140054`);
        ...
        if (units140054.length > 0) {
            ...
            console.log(`[OK] Units correctly filtered: ${correctProperty.length}/${units140054.length}`);
        } else {
            console.log('[WARN]️  No units found for property 140054 - property likely has no units');
        }
